...
import com.citybank.statementdtls.dao.config.DevDaoJavaConfig;
import com.citybank.statementdtls.dao.config.ProdDaoJavaConfig;
import com.citybank.statementdtls.dao.config.TestDaoJavaConfig;
...
public class StatementDtlsDAOImpl extends StoredProcedure implements StatementDtlsDAO, RowMapper<TrasactionDAO> {
	//private Logger logger = Logger.getLogger(StatementDtlsDAOImpl.class);
	...
	public StatementDtlsDAORes getTrasactions(StatementDtlsDAOReq daoReq)
		...
		//logger.debug("enter into spring jdbc integrations");
		String envName = environment.getProperty("enve");
		//logger.info("enveronment name " + envName);
		...
		//logger.info(" dbRespCode " + dbRespCode);
		//logger.info(" dbRespMsg " + dbRespMsg);
		if ("000".equals(dbRespCode)) {
			...
			//logger.info("transdao " + transdaoList.size());
		...
		//logger.debug("Exit from integration layer hiiiiii");
		return daoRes;
	}
