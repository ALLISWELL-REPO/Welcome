...
import com.st.bank.model.LoginUser;
...
public class UserDao extends AbstractBaseRepository {
	...
	private static final String USER_DETAILS_QUERY = "SELECT c.password from customer c where c.principal=:USER_NAME";
	...
	public LoginUser getUserDetails(String userName) {
		...
		Map<String, Object> params = Collections.singletonMap("USER_NAME", userName);
		...
		if (user != null) {
			LOG.debug("User Details is {} {}", user.getUsername(), user.getPassword());	
		}
