...
public class SortingPaginationController {
	...
	public String pagination(HttpServletRequest request, Model model) {
		...
		System.out.println("Entered into pagination controller");
		...
		List<TransactionClient> pagedList = processPagination(pageSize,
				currentPageNo);
		...
		StatementDtlsWebRes webRes = (StatementDtlsWebRes) session
				.getAttribute("webResSession");
	...
	private List<TransactionClient> processPagination(int pagesize,
			int currentPageNo) {
		System.out.println("Entered into process-pagination");
		int startIndex = 0;
		int endIndex = 0;
		...
		System.out.println("size=" + pagedList.size());
